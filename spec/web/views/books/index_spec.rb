RSpec.describe Web::Views::Books::Index do
  ...
  context 'when there are books' do
    let(:book1)     { Book.new(title: 'Refactoring Ruby', author: 'Foo') }
    let(:book2)     { Book.new(title: 'Domain Driven Design', author: 'Bar') }
    let(:exposures) { Hash[books: [book1, book2]] }

    it 'lists them all' do
      expect(rendered.scan(/class="book"/).length).to eq(2)
      expect(rendered).to include('Refactoring Ruby')
